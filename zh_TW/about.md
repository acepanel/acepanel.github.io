---
sidebar: false
prev: false
next: false
---

<script setup>
import { VPTeamMembers } from 'vitepress/theme'

const members = [
  {
    avatar: 'https://weavatar.com/avatar/18e77debb1bc0000c0b50757b8f1bebb2c3e4df3d494124f776c15dbc1ebe8a5',
    name: '耗子',
    desc: '創辦人 & CEO',
    links: [
      { icon: 'github', link: 'https://github.com/devhaozi' },
      { icon: 'bilibili', link: 'https://space.bilibili.com/8067' }
    ]
  },
  {
    avatar: 'https://weavatar.com/avatar/f6b23deadaa481f0b3ea75ad94f246881ed2326117efebad6f2799ea165779b9',
    name: '靚仔',
    desc: '技術負責人',
    links: [
      { icon: 'github', link: 'https://github.com/205125' }
    ]
  },
]
</script>

# 關於

AcePanel 是一款專業的伺服器維運管理面板，致力於為使用者提供簡單、高效且安全的伺服器管理解決方案。

| 願景                | 使命            | 價值觀             |
| ----------------- | ------------- | --------------- |
| 成為領先的伺服器管理解決方案供應商 | 讓伺服器管理變得簡單又高效 | 使用者優先、創新驅動、專業專注 |

## 團隊介紹

<VPTeamMembers size="small" :members="members" />

## 發展歷程

:::timeline 2026 - 3.0 時代
2026 年，我們推出了備受期待的 3.0 版本，帶來全新改版的使用者介面與進階功能
:::

:::timeline 2025 - 穩步前進
2025 年，我們專注於最佳化使用者體驗並提升系統穩定性，發布了 2.4 系列版本
:::

:::timeline 2024 - 全新旅程
本面板獲得了眾多贊助商的支援，並於 2024 年下半年發布了全新的 2.3 版本
:::

:::timeline 2023 - 技術累積
使用 Go 重寫了本面板，發布了 2.0 與 2.1 系列版本，累積了豐富的開發經驗
:::

:::timeline 2022 - 專案啟動
專案於 2022 年中啟動，並於年底發布了 1.0 版本
:::

## 聯絡我們

| 名稱   | 聯絡方式                                                                                       |
| ---- | ------------------------------------------------------------------------------------------ |
| 企業微信 | [點選聯絡](https://work.weixin.qq.com/kfid/kfc20ea8e38b5a4e73a)                                |
| QQ   | [826896000](https://wpa.qq.com/msgrd?v=3&uin=826896000&site=qq&menu=yes) |
| 電子郵件 | [haozi@loli.email](mailto:haozi@loli.email)                   |
| 公司地址 | 天津市武清區黃莊街道泉里路 1 號智庫大廈 206 室                                                                |
