...
本頁修改 SSH 服務設定。 它與主選單中的[終端](../ssh)不同；終端用於開啟本機或遠端終端分頁，並透過 SFTP 傳輸檔案。
...
修改埠前，請先在 **安全 > 埠規則** 和雲服務商安全組中放行新的 TCP 埠， 然後：
...
關閉密碼認證前，請先為準備使用的賬戶新增並驗證金鑰；在新的金鑰認證會話成功登入前，不要關閉當前會話。
...
修改 Root 登入策略或同時修改兩種認證方式，可能導致所有管理員都無法登入。 請保留雲服務商控制台，或另一個已經驗證且具備 sudo 許可權的賬戶。
