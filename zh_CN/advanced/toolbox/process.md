...
优先使用 `SIGTERM`，让进程有机会关闭监听并刷新数据。 `SIGKILL` 无法被进程处理，可能留下部分写入、陈旧锁或未正常关闭的数据库。 `SIGSTOP`、`SIGHUP`、`SIGUSR1` 和 `SIGUSR2` 的作用取决于具体应用，只有在服务文档明确说明预期行为时才应发送。
...
| 虚拟内存                        | 虚拟内存大小             |
| 交换内存                        | 换出的内存量             |
| 磁盘读取                        | 进程从存储设备实际读取的字节数    |
| 磁盘写入                        | 进程向存储设备实际写入的字节数    |
| 启动时间                        | 进程启动时间             |
| 可执行文件路径                     | 可执行文件的路径           |
| 工作目录                        | 当前工作目录             |
| 命令行                         | 启动进程所使用的完整命令行      |
